...
public class FileUtil {
    ...
    public static boolean deleteFileByUrl(String url) {
    	...
    	String upload = MyConst.ftp_imgs.substring(MyConst.ftp_imgs.indexOf("/") + 1, MyConst.ftp_imgs.lastIndexOf("/"));
    ...
    public static String upload(UploadFile uploadFile, String prefix) throws Exception {
    	...
    	String path = MyConst.imgUrlHead + MyConst.ftp_imgs + rename;
    	return path;
    }
...
//				}
				...
				urlmap.put("i+1",MyConst.imgUrlHead + MyConst.ftp_imgs + fileNameOnFtpServer);
			} catch (Exception e) {
				throw new UnexpectedException4View("保存图片失败", e);
			}
		}
		GFtpUtil.uploadFiles(MyConst.ftp_imgs, fileList);
	...
	public static List<String> saveToFtpServer(UploadFile... uploadFileArray) {
		...
		for (UploadFile uploadFile : uploadFileArray) {
			...
			try {
				...
				fileList.add(new FtpFile(file, fileNameOnFtpServer));
				//图片路径保存(key可以自定义，若需要关联其他表)
				list.add(MyConst.imgUrlHead + MyConst.ftp_imgs + fileNameOnFtpServer);
			} catch (Exception e) {
				throw new UnexpectedException4View("保存图片失败", e);
			}
		}
